...
public final class WalletWordDisplayScreen: ViewController {
    ...
    override public func loadDisplayNode() {
        self.displayNode = WalletWordDisplayScreenNode(presentationData: self.presentationData, wordList: self.wordList, action: { [weak self] in
            ...
            if deltaTime < minimalTimeout {
                strongSelf.present(standardTextAlertController(theme: .init(presentationData: strongSelf.presentationData), title: strongSelf.presentationData.walletStrings.Wallet_Words_NotDoneTitle, text: strongSelf.presentationData.walletStrings.Wallet_Words_NotDoneText, actions: [TextAlertAction(type: .defaultAction, title: strongSelf.presentationData.walletStrings.Wallet_Words_NotDoneOk, action: {
                    guard let strongSelf = self else {
                        return
                    }
    ...
    override public func containerLayoutUpdated(_ layout: ContainerViewLayout, transition: ContainedViewLayoutTransition) {
        ...
        (self.displayNode as! WalletWordDisplayScreenNode).containerLayoutUpdated(layout: layout, navigationHeight: self.cleanNavigationHeight, transition: transition)
    }
}
...
private final class WalletWordDisplayScreenNode: ViewControllerTracingNode, UIScrollViewDelegate {
    ...
    init(presentationData: PresentationData, wordList: [String], action: @escaping () -> Void) {
        ...
        self.animationNode.setup(source: AnimatedStickerNodeLocalFileSource(name: "WalletWordList"), width: 264, height: 264, playbackMode: .once, mode: .direct(cachePathPrefix: nil))
        self.animationNode.visibility = true
        
        let title: String = self.presentationData.walletStrings.Wallet_Words_Title
        let text: String = self.presentationData.walletStrings.Wallet_Words_Text
        let buttonText: String = self.presentationData.walletStrings.Wallet_Words_Done
    ...
    func displayToast() {
        ...
        let toastNode = ToastNode(theme: self.presentationData.theme, animationName: "WalletApologiesAccepted", text: self.presentationData.walletStrings.Wallet_Words_NotDoneResponse)
