...
final class WalletReceiveScreen: ViewController {
    ...
    public init(context: WalletContext, blockchainNetwork: LocalWalletConfiguration.ActiveNetwork, mode: WalletReceiveScreenMode) {
        ...
        self.title = self.presentationData.walletStrings.Wallet_Receive_Title
    ...
    override public func containerLayoutUpdated(_ layout: ContainerViewLayout, transition: ContainedViewLayoutTransition) {
        ...
        (self.displayNode as! WalletReceiveScreenNode).containerLayoutUpdated(layout: layout, navigationHeight: self.cleanNavigationHeight, transition: transition)
    }
...
private final class WalletReceiveScreenNode: ViewControllerTracingNode {
    ...
    init(context: WalletContext, blockchainNetwork: LocalWalletConfiguration.ActiveNetwork, presentationData: PresentationData, mode: WalletReceiveScreenMode) {
        self.context = context
        self.blockchainNetwork = blockchainNetwork
        ...
        self.qrIconNode.setup(source: AnimatedStickerNodeLocalFileSource(name: "WalletIntroStatic"), width: 240, height: 240, mode: .direct(cachePathPrefix: nil))
        ...
        let headerString = self.presentationData.walletStrings.Wallet_Receive_ShareUrlInfo
        switch self.mode {
            case .receive:
                self.textNode.attributedText = NSAttributedString(string: headerString, font: textFont, textColor: secondaryTextColor)
                self.buttonNode.title = self.presentationData.walletStrings.Wallet_Receive_ShareAddress
                self.secondaryButtonNode.setTitle(self.presentationData.walletStrings.Wallet_Receive_CreateInvoice, with: Font.regular(17.0), with: self.presentationData.theme.list.itemAccentColor, for: .normal)
            case .invoice:
                ...
                self.buttonNode.title = self.presentationData.walletStrings.Wallet_Receive_ShareInvoiceUrl
        }
