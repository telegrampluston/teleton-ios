...
final class ToastNode: ASDisplayNode {
    ...
    init(theme: PresentationTheme, animationName: String, text: String) {
        self.backgroundNode = ASDisplayNode()
        self.backgroundNode.cornerRadius = 9.0
        self.backgroundNode.clipsToBounds = true
        if case .dark = theme.rootController.keyboardColor.keyboardAppearance {
            let theme = NavigationBarTheme(rootControllerTheme: theme)
        ...
        self.animationNode.setup(source: AnimatedStickerNodeLocalFileSource(name: animationName), width: 100, height: 100, playbackMode: .once, mode: .direct(cachePathPrefix: nil))
