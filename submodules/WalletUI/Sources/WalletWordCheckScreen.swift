...
public final class WalletWordCheckScreen: ViewController {
        ...
        self.displayNode = WalletWordCheckScreenNode(presentationData: self.presentationData, mode: self.mode, possibleWordList: possibleWordList, action: { [weak self] in
            ...
            case let .verify(walletInfo, wordList, indices):
                ...
                if isCorrect {
                    ...
                    strongSelf.view.endEditing(true)
                    strongSelf.present(standardTextAlertController(theme: .init(presentationData: strongSelf.presentationData), title: strongSelf.presentationData.walletStrings.Wallet_WordCheck_IncorrectHeader, text: strongSelf.presentationData.walletStrings.Wallet_WordCheck_IncorrectText, actions: [
                        TextAlertAction(type: .defaultAction, title: strongSelf.presentationData.walletStrings.Wallet_WordCheck_TryAgain, action: {
                        }),
                        TextAlertAction(type: .genericAction, title: strongSelf.presentationData.walletStrings.Wallet_WordCheck_ViewWords, action: {
                            guard let strongSelf = self else {
                                return
                            }
                            strongSelf.dismiss()
            ...
            case .import:
                ...
                if !allWordsAreValid {
                    strongSelf.view.endEditing(true)
                    strongSelf.present(standardTextAlertController(theme: .init(presentationData: strongSelf.presentationData), title: strongSelf.presentationData.walletStrings.Wallet_WordImport_IncorrectTitle, text: strongSelf.presentationData.walletStrings.Wallet_WordImport_IncorrectText, actions: [
    ...
    override public func containerLayoutUpdated(_ layout: ContainerViewLayout, transition: ContainedViewLayoutTransition) {
        ...
        (self.displayNode as! WalletWordCheckScreenNode).containerLayoutUpdated(layout: layout, navigationHeight: self.cleanNavigationHeight, transition: transition)
    }
}
...
private final class WordCheckInputNode: ASDisplayNode, UITextFieldDelegate {
    ...
    init(theme: PresentationTheme, index: Int, possibleWordList: [String], previous: @escaping (WordCheckInputNode) -> Void, next: @escaping (WordCheckInputNode, Bool) -> Void, isLast: Bool, focused: @escaping (WordCheckInputNode) -> Void, pasteWords: @escaping ([String]) -> Void) {
        ...
        self.clearButtonNode.setImage(generateClearIcon(color: theme.actionSheet.inputClearButtonColor.withAlphaComponent(0.8)), for: [])
...
private final class WordView: UIView {
    ...
    init(theme: PresentationTheme, string: String, tapped: @escaping () -> Void) {
        ...
        switch theme.rootController.keyboardColor.keyboardAppearance {
        case .light, .default:
...
private final class WalletWordCheckScreenNode: ViewControllerTracingNode, UIScrollViewDelegate {
    ...
    init(presentationData: PresentationData, mode: WalletWordCheckMode, possibleWordList: [String], action: @escaping () -> Void, secondaryAction: @escaping () -> Void) {
        ...
        case let .verify(_, _, indices):
            wordIndices = indices
            title = self.presentationData.walletStrings.Wallet_WordCheck_Title
            ...
            text = parseMarkdownIntoAttributedString(self.presentationData.walletStrings.Wallet_WordCheck_Text("\(wordIndices[0] + 1)", "\(wordIndices[1] + 1)", "\(wordIndices[2] + 1)").0, attributes: MarkdownAttributes(body: body, bold: bold, link: body, linkAttribute: { _ in nil }), textAlignment: .center)
            
            buttonText = self.presentationData.walletStrings.Wallet_WordCheck_Continue
            secondaryActionText = ""
            self.animationNode.setup(source: AnimatedStickerNodeLocalFileSource(name: "WalletWordCheck"), width: 238, height: 238, playbackMode: .once, mode: .direct(cachePathPrefix: nil))
            self.animationNode.visibility = true
        case .import:
            title = self.presentationData.walletStrings.Wallet_WordImport_Title
            text = NSAttributedString(string: self.presentationData.walletStrings.Wallet_WordImport_Text, font: Font.regular(16.0), textColor: self.presentationData.theme.list.itemPrimaryTextColor)
            buttonText = self.presentationData.walletStrings.Wallet_WordImport_Continue
            secondaryActionText = self.presentationData.walletStrings.Wallet_WordImport_CanNotRemember
