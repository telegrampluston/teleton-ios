...
private enum WalletSendScreenEntry: ItemListNodeEntry {
    ...
    case balance(PresentationTheme, String, String, Bool)
    ...
    case comment(PresentationTheme, String, String, Bool)
    ...
    func item(presentationData: ItemListPresentationData, arguments: Any) -> ListViewItem {
        ...
        case let .addressHeader(_, text):
            return ItemListSectionHeaderItem(presentationData: presentationData, text: text, sectionId: self.section)
        case let .address(_, placeholder, address):
            return ItemListMultilineInputItem(presentationData: presentationData, text: address, placeholder: placeholder, maxLength: .init(value: walletAddressLength, display: false), sectionId: self.section, style: .blocks, capitalization: false, autocorrection: false, returnKeyType: .next, minimalHeight: 68.0, textUpdated: { text in
        ...
        case let .addressInfo(_, text):
            return ItemListTextItem(presentationData: presentationData, text: .markdown(text), sectionId: self.section)
        case let .commentHeader(_, text):
            return ItemListSectionHeaderItem(presentationData: presentationData, text: text, sectionId: self.section)
        case let .comment(theme, placeholder, value, sendEnabled):
            return ItemListMultilineInputItem(presentationData: presentationData, text: value, placeholder: placeholder, maxLength: ItemListMultilineInputItemTextLimit(value: walletTextLimit, display: true, mode: .bytes), sectionId: self.section, style: .blocks, returnKeyType: .send, textUpdated: { text in
...
private func walletSendScreenEntries(presentationData: PresentationData, balance: Int64?, state: WalletSendScreenState, sendEnabled: Bool) -> [WalletSendScreenEntry] {
    if balance == nil {
        return []
    }
    ...
    entries.append(.balance(presentationData.theme, presentationData.walletStrings.Wallet_Send_Balance("").0, formatBalanceText(balance, decimalSeparator: presentationData.dateTimeFormat.decimalSeparator), balance == 0 || (amount > 0 && balance < amount)))
    
    entries.append(.addressHeader(presentationData.theme, presentationData.walletStrings.Wallet_Send_AddressHeader))
    entries.append(.address(presentationData.theme, presentationData.walletStrings.Wallet_Send_AddressText, state.address))
    entries.append(.addressInfo(presentationData.theme, presentationData.walletStrings.Wallet_Send_AddressInfo))
        
    entries.append(.commentHeader(presentationData.theme, presentationData.walletStrings.Wallet_Receive_CommentHeader))
    entries.append(.comment(presentationData.theme, presentationData.walletStrings.Wallet_Receive_CommentInfo, state.comment, sendEnabled))
...
public func walletSendScreen(context: WalletContext, randomId: Int64, walletInfo: WalletInfo, blockchainNetwork: LocalWalletConfiguration.ActiveNetwork, address: String? = nil, amount: Int64? = nil, comment: String? = nil) -> ViewController {    
    ...
    let arguments = WalletSendScreenArguments(context: context, updateState: { f in
        ...
        let proceed: () -> Void = {
            ...
            |> deliverOnMainQueue).start(next: { [weak controller] verificationResult in
                ...
                let title = NSAttributedString(string: presentationData.walletStrings.Wallet_Send_Confirmation, font: Font.semibold(17.0), textColor: presentationData.theme.list.itemPrimaryTextColor)
                ...
                let (text, ranges) = presentationData.walletStrings.Wallet_Send_ConfirmationText(formatBalanceText(amount, decimalSeparator: presentationData.dateTimeFormat.decimalSeparator), formattedAddress, "\(formatBalanceText(feeAmount, decimalSeparator: presentationData.dateTimeFormat.decimalSeparator))")
                ...
                let themeSignal = context.presentationDataSignal |> map { AlertControllerTheme(presentationData: $0) }
                let controller = richTextAlertController(alertContext: AlertControllerContext(theme: .init(presentationData: context.presentationData), themeSignal: themeSignal), title: title, text: attributedText, actions: [TextAlertAction(type: .genericAction, title: presentationData.walletStrings.Wallet_Navigation_Cancel, action: {
                    dismissAlertImpl?(true)
                }), TextAlertAction(type: .defaultAction, title: presentationData.walletStrings.Wallet_Send_ConfirmationConfirm, action: {
                ...
                switch error {
                case .generic:
                    text = presentationData.walletStrings.Wallet_UnknownError
                case .network:
                    title = presentationData.walletStrings.Wallet_Send_NetworkErrorTitle
                    text = presentationData.walletStrings.Wallet_Send_NetworkErrorText
                case .notEnoughFunds:
                    title = presentationData.walletStrings.Wallet_Send_ErrorNotEnoughFundsTitle
                    text = presentationData.walletStrings.Wallet_Send_ErrorNotEnoughFundsText
                case .messageTooLong:
                    text = presentationData.walletStrings.Wallet_UnknownError
                case .invalidAddress:
                    text = presentationData.walletStrings.Wallet_Send_ErrorInvalidAddress
                case .secretDecryptionFailed:
                    text = presentationData.walletStrings.Wallet_Send_ErrorDecryptionFailed
                case .destinationIsNotInitialized:
                    text = presentationData.walletStrings.Wallet_UnknownError
                }
                let controller = standardTextAlertController(theme: .init(presentationData: presentationData), title: title, text: text, actions: [TextAlertAction(type: .defaultAction, title: presentationData.walletStrings.Wallet_Alert_OK, action: {
                })])
                presentControllerImpl?(controller, nil)
            })
        }
        ...
        if destinationAddress == walletInfo.address {
            presentControllerImpl?(standardTextAlertController(theme: .init(presentationData: presentationData), title: presentationData.walletStrings.Wallet_Send_OwnAddressAlertTitle, text: presentationData.walletStrings.Wallet_Send_OwnAddressAlertText, actions: [
                TextAlertAction(type: .genericAction, title: presentationData.walletStrings.Wallet_Alert_Cancel, action: {
                }),
                TextAlertAction(type: .defaultAction, title: presentationData.walletStrings.Wallet_Send_OwnAddressAlertProceed, action: {
                    proceed()
                })
            ]), nil)
        } else {
    ...
    |> map { presentationData, walletState, state -> (ItemListControllerState, (ItemListNodeState, Any)) in
        let leftNavigationButton = ItemListNavigationButton(content: .text(presentationData.walletStrings.Wallet_Navigation_Cancel), style: .regular, enabled: true, action: {
            dismissImpl?()
        })
        ...
        if let walletState = walletState {
            ...
            rightNavigationButton = ItemListNavigationButton(content: .text(presentationData.walletStrings.Wallet_Send_Send), style: .bold, enabled: sendEnabled, action: {
                arguments.proceed()
            })
        } else {
        ...
        let controllerState = ItemListControllerState(presentationData: .init(presentationData), title: .text(presentationData.walletStrings.Wallet_Send_Title), leftNavigationButton: leftNavigationButton, rightNavigationButton: rightNavigationButton, backNavigationButton: ItemListBackButton(title: presentationData.walletStrings.Wallet_Navigation_Back), animateChanges: false)
        let listState = ItemListNodeState(presentationData: .init(presentationData), entries: walletSendScreenEntries(presentationData: presentationData, balance: walletState?.effectiveAvailableBalance, state: state, sendEnabled: sendEnabled), style: .blocks, focusItemTag: focusItemTag, emptyStateItem: emptyItem, animateChanges: false)
        
        return (controllerState, (listState, arguments))
    }
    let updatedPresentationData = context.presentationDataSignal |> map { ItemListPresentationData($0) }
    let controller = WalletSendScreenImpl(presentationData: .init(context.presentationData), updatedPresentationData: updatedPresentationData, state: signal, tabBarItem: nil)
