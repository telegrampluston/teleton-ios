...
class WalletInfoEmptyItem: ListViewItem {
    ...
    init(theme: PresentationTheme, strings: WalletStrings, address: String, loading: Bool, displayAddressContextMenu: @escaping (ASDisplayNode, CGRect) -> Void) {
        self.theme = theme
        ...
        return { [weak self] item, params in
            ...
            return (layout, {
                ...
                if strongSelf.item?.loading != item.loading {
                    if item.loading {
                        strongSelf.animationNode.setup(source: AnimatedStickerNodeLocalFileSource(name: "WalletInitializing"), width: 280, height: 280, playbackMode: .loop, mode: .direct(cachePathPrefix: nil))
                        strongSelf.animationNode.visibility = true
                    } else {
                        strongSelf.animationNode.setup(source: AnimatedStickerNodeLocalFileSource(name: "WalletEmpty"), width: 280, height: 280, playbackMode: .once, mode: .direct(cachePathPrefix: nil))
                        strongSelf.animationNode.visibility = true
                    }
                }
