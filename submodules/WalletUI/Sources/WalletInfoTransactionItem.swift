...
private func stringForMessageTimestamp(timestamp: Int32, dateTimeFormat: PresentationDateTimeFormat, local: Bool = true) -> String {
    var t = Int(timestamp)
    var timeinfo = tm()
    if local {
...
class WalletInfoTransactionItem: ListViewItem {
    ...
    init(theme: PresentationTheme, strings: WalletStrings, dateTimeFormat: PresentationDateTimeFormat, walletTransaction: WalletInfoTransaction, action: @escaping () -> Void) {
        self.theme = theme
...
class WalletInfoTransactionItemNode: ListViewItemNode {
    ...
    override func headers() -> [ListViewItemHeader]? {
        guard let header = self.item?.header else { return [] }
        return [header]
    }
}
...
private final class WalletInfoTransactionDateHeader: ListViewItemHeader {
    ...
    init(timestamp: Int32, theme: PresentationTheme, strings: WalletStrings) {
    ...
    func combinesWith(other: ListViewItemHeader) -> Bool {
        if let other = other as? WalletInfoTransactionDateHeader, other.id == self.id {
            return true
        } else {
            return false
        }
...
final class WalletInfoTransactionDateHeaderNode: ListViewItemHeaderNode {
    ...
    init(theme: PresentationTheme, strings: WalletStrings, roundedTimestamp: Int32) {
        self.theme = theme
        self.strings = strings
    ...
    func updateThemeAndStrings(theme: PresentationTheme, strings: WalletStrings) {
        self.theme = theme
