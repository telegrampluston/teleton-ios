...
class WalletAmountItem: ListViewItem, ItemListItem {
    ...
    init(theme: PresentationTheme, amount: String, sectionId: ItemListSectionId, textUpdated: @escaping (String) -> Void, shouldUpdateText: @escaping (String) -> Bool = { _ in return true }, processPaste: ((String) -> String)? = nil, updatedFocus: ((Bool) -> Void)? = nil, tag: ItemListItemTag? = nil) {
        self.theme = theme
        self.amount = amount
...
private let verticalOffset: CGFloat = .zero
...
class WalletAmountItemNode: ListViewItemNode, UITextFieldDelegate, ItemListItemNode, ItemListItemFocusableNode {
    ...
    init() {
        ...
        self.iconNode.setup(source: AnimatedStickerNodeLocalFileSource(name: "WalletIntroStatic"), width: 120, height: 120, mode: .direct(cachePathPrefix: nil))
