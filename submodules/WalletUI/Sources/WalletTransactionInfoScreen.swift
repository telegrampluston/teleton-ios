...
private func stringForFullDate(timestamp: Int32, strings: WalletStrings, dateTimeFormat: PresentationDateTimeFormat) -> String {
...
//    private let blockchainNetwork: LocalWalletConfiguration.ActiveNetwork Fork commented
    let blockchainNetwork: LocalWalletConfiguration.ActiveNetwork // Fork
    ...
    public init(context: WalletContext, blockchainNetwork: LocalWalletConfiguration.ActiveNetwork, walletInfo: WalletInfo?, walletTransaction: WalletInfoTransaction, walletState: Signal<(CombinedWalletState, Bool), NoError>, enableDebugActions: Bool, decryptionKeyUpdated: @escaping (WalletTransactionDecryptionKey) -> Void) {
        ...
        self.navigationItem.rightBarButtonItem = UIBarButtonItem(title: self.presentationData.walletStrings.Wallet_Navigation_Done, style: .done, target: self, action: #selector(self.donePressed))
    ...
    override public func loadDisplayNode() {
        ...
        (self.displayNode as! WalletTransactionInfoScreenNode).displayFeesTooltip = { [weak self] node, rect in
            ...
            if let _ = strongSelf.context.feeInfoUrl {
                ...
                let link = MarkdownAttributeSet(font: Font.regular(14.0), textColor: strongSelf.context.presentationData.theme.list.itemAccentColor, additionalAttributes: [:])
                text = parseMarkdownIntoAttributedString(strongSelf.context.presentationData.walletStrings.Wallet_TransactionInfo_StorageFeeInfoUrl, attributes: MarkdownAttributes(body: body, bold: body, link: link, linkAttribute: { _ in nil }), textAlignment: .center)
            } else {
                text = NSAttributedString(string: strongSelf.context.presentationData.walletStrings.Wallet_TransactionInfo_StorageFeeInfo, font: Font.regular(14.0), textColor: .white, paragraphAlignment: .center)
            }
    ...
    override public func containerLayoutUpdated(_ layout: ContainerViewLayout, transition: ContainedViewLayoutTransition) {
        ...
        (self.displayNode as! WalletTransactionInfoScreenNode).containerLayoutUpdated(layout: layout, navigationHeight: self.cleanNavigationHeight, transition: transition)
    }
...
private final class WalletTransactionInfoScreenNode: ViewControllerTracingNode, UIScrollViewDelegate {
    ...
    // MARK: - Fork begin
    private let transactionHashNode: ImmediateTextNode
    private let transactionHashButton: HighlightableButtonNode
    // MARK: Fork end -
    ...
    init(controller: WalletTransactionInfoScreen, context: WalletContext, presentationData: PresentationData, walletTransaction: WalletInfoTransaction, decryptionKeyUpdated: @escaping (WalletTransactionDecryptionKey) -> Void) {
        ...
        self.navigationBackgroundNode.backgroundColor = defaultTheme.backgroundColor
        self.navigationBackgroundNode.alpha = 0.0
        self.navigationSeparatorNode = ASDisplayNode()
        self.navigationSeparatorNode.backgroundColor = defaultTheme.separatorColor
        ...
        self.buttonNode = SolidRoundedButtonNode(title: "", icon: nil, theme: SolidRoundedButtonTheme(backgroundColor: self.presentationData.theme.list.itemCheckColors.fillColor, foregroundColor: self.presentationData.theme.list.itemCheckColors.foregroundColor), height: 50.0, cornerRadius: 10.0, gloss: false)
        ...
        // MARK: - Fork begin
        self.transactionHashNode = ImmediateTextNode()
        self.transactionHashNode.textAlignment = .center
        self.transactionHashNode.maximumNumberOfLines = 4
        self.transactionHashNode.lineSpacing = 0.35
        self.transactionHashNode.displaysAsynchronously = false
        self.transactionHashButton = HighlightableButtonNode()
        // MARK: Fork End -
        ...
        self.titleNode.attributedText = NSAttributedString(string: self.presentationData.walletStrings.Wallet_TransactionInfo_Title, font: titleFont, textColor: textColor)
        
        self.timeNode.attributedText = NSAttributedString(string: stringForFullDate(timestamp: Int32(clamping: timestamp), strings: self.presentationData.walletStrings, dateTimeFormat: self.presentationData.dateTimeFormat), font: subtitleFont, textColor: seccondaryTextColor)
        ...
        if transferredValue <= 0 {
            ...
            amountColor = self.presentationData.theme.list.itemDestructiveColor
        } else {
            amountString = "\(formatBalanceText(transferredValue, decimalSeparator: self.presentationData.dateTimeFormat.decimalSeparator))"
            amountColor = self.presentationData.theme.chatList.secretTitleColor
        }
        ...
        if case let .completed(transaction) = walletTransaction {
            if transaction.otherFee != 0 {
                feesString.append(formatBalanceText(transaction.otherFee, decimalSeparator: presentationData.dateTimeFormat.decimalSeparator) + " \(presentationData.walletStrings.Wallet_TransactionInfo_OtherFeeHeader.lowercased())")
            }
            if transaction.storageFee != 0 {
                if !feesString.isEmpty {
                    feesString.append("\n")
                }
                feesString.append(formatBalanceText(transaction.storageFee, decimalSeparator: presentationData.dateTimeFormat.decimalSeparator) + " \(presentationData.walletStrings.Wallet_TransactionInfo_StorageFeeHeader.lowercased())")
            }
        ...
        case .completed(let walletTransaction):
            let mutableAttributedString = NSMutableAttributedString(
                string: "Tx hash: \(walletTransaction.transactionHash)",
                font: subtitleFont,
                textColor: seccondaryTextColor
            )
            let range = mutableAttributedString.mutableString.range(of: walletTransaction.transactionHash)
            mutableAttributedString.addAttributes(
                [.foregroundColor: presentationData.theme.list.itemAccentColor],
                range: range
            )
            self.transactionHashNode.attributedText = mutableAttributedString
        case .pending:
            self.transactionHashNode.isHidden = true
            self.transactionHashButton.isHidden = true
        }
        self.transactionHashButton.highligthedChanged = { [weak self] highlighted in
            if let strongSelf = self {
                if highlighted {
                    strongSelf.transactionHashNode.layer.removeAnimation(forKey: "opacity")
                    strongSelf.transactionHashNode.alpha = 0.4
                } else {
                    strongSelf.transactionHashNode.alpha = 1.0
                    strongSelf.transactionHashNode.layer.animateAlpha(from: 0.4, to: 1.0, duration: 0.2)
                }
            }
        }
        self.transactionHashButton.addTarget(self, action: #selector(self.hashPressed), forControlEvents: .touchUpInside)
        addSubnode(transactionHashNode)
        addSubnode(transactionHashButton)
        // MARK: Fork end -
        
        var commentBackgroundColor = presentationData.theme.chat.message.incoming.bubble.withoutWallpaper.fill.first!
        ...
        self.commentBackgroundNode.image = messageBubbleImage(incoming: transferredValue > 0, fillColor: commentBackgroundColor, strokeColor: presentationData.theme.chat.message.incoming.bubble.withoutWallpaper.fill.first!)
        let (plainText, textIsEncrypted) = extractDescription(walletTransaction)
        self.commentTextNode.attributedText = NSAttributedString(string: plainText, font:  textIsEncrypted ? Font.monospace(17.0) : Font.regular(17.0), textColor: presentationData.theme.chat.message.incoming.primaryTextColor)
        ...
        if let address = singleAddress {
            ...
            self.buttonNode.title = presentationData.walletStrings.Wallet_TransactionInfo_SendGrams
    ...
    // MARK: - Fork begin
    @objc private func hashPressed() {
        if case .completed(let transaction) = walletTransaction,
           let network = controller?.blockchainNetwork {
            var disposable: Disposable?
            disposable = (
                context.storage.getWalletRecords() |> deliverOnMainQueue
                ...
                case .ready(info: let info, exportCompleted: _, state: _):
                    let url = transaction.exploreTxFor(address: info.address, activeNetwork: network)
                    self?.context.openUrl(url)
    ...
    private func updateTitle(transition: ContainedViewLayoutTransition) {
        ...
        // MARK: - Fork begin
        let transactionHashWidth = feesSize.width.isZero ? layout.size.width - sideInset * 2.0 : feesSize.width
        let transactionOriginX = feesSize.width.isZero ? floor((layout.size.width - transactionHashWidth) / 2.0) : feesFrame.minX
        let transactionHashSize = transactionHashNode.updateLayout(
            CGSize(width: transactionHashWidth, height: .greatestFiniteMagnitude)
        )
        let transactionHashFrame = CGRect(
            origin: CGPoint(x: transactionOriginX, y: feesFrame.maxY),
            size: transactionHashSize
        )
        transition.updateFrame(node: transactionHashNode, frame: transactionHashFrame)
        transition.updateFrame(node: transactionHashButton, frame: transactionHashFrame)
        // MARK: Fork end -
