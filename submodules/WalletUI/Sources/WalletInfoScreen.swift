...
public final class WalletInfoScreen: ViewController {
    ...
    public init(context: WalletContext, walletInfo: WalletInfo, blockchainNetwork: LocalWalletConfiguration.ActiveNetwork, enableDebugActions: Bool) {
        ...
        let navigationBarTheme = NavigationBarTheme(buttonColor: .white, disabledButtonColor: .white, primaryTextColor: .white, backgroundColor: .clear, enableBackgroundBlur: false, separatorColor: .clear, badgeBackgroundColor: defaultTheme.badgeBackgroundColor, badgeStrokeColor: defaultTheme.badgeStrokeColor, badgeTextColor: defaultTheme.badgeTextColor)
        
        super.init(navigationBarPresentationData: NavigationBarPresentationData(theme: navigationBarTheme, strings: NavigationBarStrings(back: self.presentationData.walletStrings.Wallet_Navigation_Back, close: self.presentationData.walletStrings.Wallet_Navigation_Close)))
        ...
        self.navigationItem.backBarButtonItem = UIBarButtonItem(title: self.presentationData.walletStrings.Wallet_Navigation_Back, style: .plain, target: nil, action: nil)
        // MARK: - Fork begin
        let explorer = UIBarButtonItem(image: generateTintedImage(image: UIImage(bundleImageName: "Wallet/NavigationExplorerIcon"), color: .white), style: .plain, target: self, action: #selector(self.explorer))
        let settingsButton = UIBarButtonItem(image: generateTintedImage(image: UIImage(bundleImageName: "Wallet/NavigationSettingsIcon"), color: .white), style: .plain, target: self, action: #selector(self.settingsPressed))
        self.navigationItem.setRightBarButtonItems([explorer, settingsButton], animated: true)
        // MARK: Fork end -
//        self.navigationItem.rightBarButtonItem = UIBarButtonItem(image: generateTintedImage(image: UIImage(bundleImageName: "Wallet/NavigationSettingsIcon"), color: .white), style: .plain, target: self, action: #selector(self.settingsPressed)) Fork commenteed
    ...
    // MARK: - Fork begin
    @objc private func explorer() {
        context.exploreAddress()
    }
    // MARK: Fork end -
    ...
    override public func loadDisplayNode() {
        self.displayNode = WalletInfoScreenNode(context: self.context, presentationData: self.presentationData, walletInfo: self.walletInfo, blockchainNetwork: self.blockchainNetwork, sendAction: { [weak self] in
            ...
            if (strongSelf.displayNode as! WalletInfoScreenNode).reloadingState {
                strongSelf.present(standardTextAlertController(theme: .init(presentationData: strongSelf.presentationData), title: nil, text: strongSelf.presentationData.walletStrings.Wallet_Send_SyncInProgress, actions: [
                    TextAlertAction(type: .defaultAction, title: strongSelf.presentationData.walletStrings.Wallet_Alert_OK, action: {
                    })
                ]), in: .window(.root))
            } else if !combinedState.pendingTransactions.isEmpty {
                strongSelf.present(standardTextAlertController(theme: .init(presentationData: strongSelf.presentationData), title: nil, text: strongSelf.presentationData.walletStrings.Wallet_Send_TransactionInProgress, actions: [
                    TextAlertAction(type: .defaultAction, title: strongSelf.presentationData.walletStrings.Wallet_Alert_OK, action: {
                    })
                ]), in: .window(.root))
            } else {
...
//            })) Fork commented
            let controller = WalletTransactionInfoScreen(context: strongSelf.context, blockchainNetwork: strongSelf.blockchainNetwork, walletInfo: strongSelf.walletInfo, walletTransaction: transaction, walletState: (strongSelf.displayNode as! WalletInfoScreenNode).statePromise.get(), enableDebugActions: strongSelf.enableDebugActions, decryptionKeyUpdated: { key in
                guard let strongSelf = self else {
                    return
                }
                (strongSelf.displayNode as! WalletInfoScreenNode).updateTransactionDecryptionKey(key)
    ...
    private func linkToTelegram() {
        let cancelAction = TextAlertAction(
            type: .genericAction,
            title: presentationData.strings.Common_Cancel,
            action: { }
        )
        let linkAction = TextAlertAction(
            type: .defaultAction,
            title: presentationData.strings.Common_OK) { [weak self] in
                self?.context.startLinkWallet()
            }
        let alert = standardTextAlertController(
            theme: .init(presentationData: presentationData),
            title: presentationData.strings.Wallet_Info_LinkAlert_Title,
            text: presentationData.strings.Wallet_Info_LinkAlert_Text,
            actions: [cancelAction, linkAction]
        )
    ...
    override public func containerLayoutUpdated(_ layout: ContainerViewLayout, transition: ContainedViewLayoutTransition) {
        ...
        (self.displayNode as! WalletInfoScreenNode).containerLayoutUpdated(layout: layout, navigationHeight: self.cleanNavigationHeight, transition: transition)
    }
}
...
final class WalletInfoBalanceNode: ASDisplayNode {
    ...
    init(dateTimeFormat: PresentationDateTimeFormat) {
        ...
        self.balanceIconNode.setup(source: AnimatedStickerNodeLocalFileSource(name: "WalletIntroStatic"), width: 120, height: 120, mode: .direct(cachePathPrefix: nil))
    ...
    func update(width: CGFloat, scaleTransition: CGFloat, transition: ContainedViewLayoutTransition) -> CGFloat {
        ...
        if !isBalanceEmpty != self.balanceIconNodeIsStatic {
            ...
            if isBalanceEmpty {
                self.balanceIconNode.setup(source: AnimatedStickerNodeLocalFileSource(name: "WalletIntroLoading"), width: 120, height: 120, mode: .direct(cachePathPrefix: nil))
...
private final class WalletInfoHeaderNode: ASDisplayNode {
    ...
    private let linkButtonNode: SolidRoundedButtonNode
    private let headerBackgroundNode: ASDisplayNode
    private let headerCornerNode: ASImageNode
    
    init(presentationData: PresentationData, hasActions: Bool, sendAction: @escaping () -> Void, receiveAction: @escaping () -> Void, linkAction: @escaping () -> Void) {
        ...
        self.balanceSubtitleNode.attributedText = NSAttributedString(string: presentationData.walletStrings.Wallet_Info_YourBalance, font: Font.regular(13), textColor: UIColor(white: 1.0, alpha: 0.6))
        
        self.balanceSubtitleIconNode = AnimatedStickerNode()
        self.balanceSubtitleIconNode.isHidden = true
        self.balanceSubtitleIconNode.setup(source: AnimatedStickerNodeLocalFileSource(name: "WalletIntroStatic"), width: 36, height: 36, mode: .direct(cachePathPrefix: nil))
        ...
        let buttonReceiveForegroundColor = UIColor.systemGreen
        let buttonSendForegroundColor = UIColor.systemRed
        let transparentButtonBackgroundColor = UIColor.white.withAlphaComponent(0.2)
        let transparentButtonForegroundColor = UIColor.white
        self.receiveButtonNode = SolidRoundedButtonNode(title: presentationData.walletStrings.Wallet_Info_Receive, icon: UIImage(bundleImageName: "Wallet/ReceiveButtonIcon"), theme: SolidRoundedButtonTheme(backgroundColor: buttonBackgroundColor, foregroundColor: buttonReceiveForegroundColor), height: 50.0, cornerRadius: 10.0, gloss: false)
        self.receiveGramsButtonNode = SolidRoundedButtonNode(title: presentationData.walletStrings.Wallet_Info_ReceiveGrams, icon: UIImage(bundleImageName: "Wallet/ReceiveButtonIcon"), theme: SolidRoundedButtonTheme(backgroundColor: buttonBackgroundColor, foregroundColor: buttonReceiveForegroundColor), height: 50.0, cornerRadius: 10.0, gloss: false)
        self.sendButtonNode = SolidRoundedButtonNode(title: presentationData.walletStrings.Wallet_Info_Send, icon: UIImage(bundleImageName: "Wallet/SendButtonIcon"), theme: SolidRoundedButtonTheme(backgroundColor: buttonBackgroundColor, foregroundColor: buttonSendForegroundColor), height: 50.0, cornerRadius: 10.0, gloss: false)
        self.linkButtonNode = SolidRoundedButtonNode(title: presentationData.strings.Wallet_Info_LinkButton_Title, theme: SolidRoundedButtonTheme(backgroundColor: transparentButtonBackgroundColor, foregroundColor: transparentButtonForegroundColor), height: 50.0, cornerRadius: 10.0, gloss: false)
        
        self.refreshNode = WalletRefreshNode(strings: presentationData.walletStrings, dateTimeFormat: presentationData.dateTimeFormat)
        ...
        if hasActions {
            ...
            self.addSubnode(self.linkButtonNode)
        }
    ...
    func update(size: CGSize, navigationHeight: CGFloat, offset: CGFloat, transition: ContainedViewLayoutTransition, isScrolling: Bool) {
        ...
        let headerHeight: CGFloat = 1000.0 + buttonHeight + sideInset * 2
        ...
        let secondFullButtonFrame = CGRect(origin: CGPoint(x: sideInset, y: buttonOffset - sideInset - buttonHeight), size: CGSize(width: size.width - sideInset * 2.0, height: buttonHeight))
        let leftButtonFrame = CGRect(origin: CGPoint(x: sideInset, y: secondFullButtonFrame.origin.y - buttonHeight - sideInset), size: CGSize(width: floor((size.width - sideInset * 3.0) / 2.0), height: buttonHeight))
        ...
        let fullButtonFrame = CGRect(origin: CGPoint(x: sideInset, y: leftButtonFrame.minY), size: CGSize(width: size.width - sideInset * 2.0, height: buttonHeight))
        ...
        transition.updateFrame(node: self.linkButtonNode, frame: secondFullButtonFrame)
        transition.updateAlpha(node: self.linkButtonNode, alpha: buttonAlpha, beginWithCurrentState: true)
        let _ = self.linkButtonNode.updateLayout(width: secondFullButtonFrame.width, transition: transition)
    }
    ...
    override func hitTest(_ point: CGPoint, with event: UIEvent?) -> UIView? {
        ...
        if let result = self.linkButtonNode.hitTest(self.view.convert(point, to: self.linkButtonNode.view), with: event) {
            return result
        }
        return nil
    }
    ...
    func becameReady(animated: Bool) {
        if animated {
            ...
            self.linkButtonNode.layer.animateAlpha(from: 0.0, to: 1.0, duration: 0.2)
...
private enum WalletInfoListEntry: Equatable, Comparable, Identifiable {
    ...
    func item(theme: PresentationTheme, strings: WalletStrings, dateTimeFormat: PresentationDateTimeFormat, action: @escaping (WalletInfoTransaction) -> Void, displayAddressContextMenu: @escaping (ASDisplayNode, CGRect) -> Void) -> ListViewItem {
        switch self {
...
private func preparedTransition(from fromEntries: [WalletInfoListEntry], to toEntries: [WalletInfoListEntry], presentationData: PresentationData, action: @escaping (WalletInfoTransaction) -> Void, displayAddressContextMenu: @escaping (ASDisplayNode, CGRect) -> Void) -> WalletInfoListTransaction {
    let (deleteIndices, indicesAndItems, updateIndices) = mergeListsStableWithUpdates(leftList: fromEntries, rightList: toEntries)
    
    let deletions = deleteIndices.map { ListViewDeleteItem(index: $0, directionHint: nil) }
    let insertions = indicesAndItems.map { ListViewInsertItem(index: $0.0, previousIndex: $0.2, item: $0.1.item(theme: presentationData.theme, strings: presentationData.walletStrings, dateTimeFormat: presentationData.dateTimeFormat, action: action, displayAddressContextMenu: displayAddressContextMenu), directionHint: nil) }
    let updates = updateIndices.map { ListViewUpdateItem(index: $0.0, previousIndex: $0.2, item: $0.1.item(theme: presentationData.theme, strings: presentationData.walletStrings, dateTimeFormat: presentationData.dateTimeFormat, action: action, displayAddressContextMenu: displayAddressContextMenu), directionHint: nil) }
...
private final class WalletInfoScreenNode: ViewControllerTracingNode {
    ...
    init(context: WalletContext, presentationData: PresentationData, walletInfo: WalletInfo, blockchainNetwork: LocalWalletConfiguration.ActiveNetwork, sendAction: @escaping () -> Void, receiveAction: @escaping () -> Void, linkAction: @escaping () -> Void, openTransaction: @escaping (WalletInfoTransaction) -> Void, present: @escaping (ViewController, Any?) -> Void) {
        self.context = context
        ...
        self.headerNode = WalletInfoHeaderNode(presentationData: presentationData, hasActions: true, sendAction: sendAction, receiveAction: receiveAction, linkAction: linkAction)
...
//        self.listNode.keepMinimalScrollHeightWithTopInset = ListViewKeepMinimalScrollHeightWithTopInset(value: .headerInsets, keepForSingleItem: false)
        ...
        self.listNode.endedInteractiveDragging = { _ in
            isScrolling = false
        }
        ...
        self.listNode.updateFloatingHeaderOffset = { [weak self] offset, listTransition in
            ...
            let headerHeight: CGFloat = navigationHeight + 260.0 + 50.0 + (16 * 2)
    ...
    private func refreshTransactions() {
        ...
        |> deliverOnMainQueue).start(next: { [weak self] value in
            ...
            switch error {
            case .generic:
                text = strongSelf.presentationData.walletStrings.Wallet_Info_RefreshErrorText
            case .network:
                text = strongSelf.presentationData.walletStrings.Wallet_Info_RefreshErrorNetworkText
            }
            strongSelf.present(standardTextAlertController(theme: .init(presentationData: strongSelf.presentationData), title: strongSelf.presentationData.walletStrings.Wallet_Info_RefreshErrorTitle, text: text, actions: [
                TextAlertAction(type: .defaultAction, title: strongSelf.presentationData.walletStrings.Wallet_Alert_OK, action: {
                })
        ...
        if let combinedState = combinedState {
            ...
            if let unlockedBalance = combinedState.walletState.unlockedBalance {
                ...
                if lockedBalance <= 0 {
                    ...
                    case .mainNet:
                        balanceLabel = self.presentationData.walletStrings.Wallet_Info_YourBalance
                    case .testNet:
                        balanceLabel = self.presentationData.walletStrings.Wallet_Info_YourBalance + " (Testnet)"
                    }
    ...
    private func replaceEntries(_ updatedEntries: [WalletInfoListEntry]) {
        let transaction = preparedTransition(from: self.currentEntries ?? [], to: updatedEntries, presentationData: self.presentationData, action: { [weak self] transaction in
            ...
            let contextMenuController = ContextMenuController(actions: [ContextMenuAction(content: .text(title: strongSelf.presentationData.walletStrings.Wallet_ContextMenuCopy, accessibilityLabel: strongSelf.presentationData.walletStrings.Wallet_ContextMenuCopy), action: {
...
private final class WalletApplicationSplashScreenNode: ASDisplayNode {
    ...
    func containerLayoutUpdated(layout: ContainerViewLayout, navigationHeight: CGFloat, transition: ContainedViewLayoutTransition) {
        ...
        let headerHeight = navigationHeight + 260.0 + 50.0 + (16 * 2)
...
public final class WalletApplicationSplashScreen: ViewController {
    ...
    public init(theme: PresentationTheme) {
        ...
        let defaultTheme = NavigationBarTheme(rootControllerTheme: theme)
        let navigationBarTheme = NavigationBarTheme(buttonColor: .white, disabledButtonColor: .white, primaryTextColor: .white, backgroundColor: .clear, enableBackgroundBlur: false, separatorColor: .clear, badgeBackgroundColor: defaultTheme.badgeBackgroundColor, badgeStrokeColor: defaultTheme.badgeStrokeColor, badgeTextColor: defaultTheme.badgeTextColor)
    ...
    override public func containerLayoutUpdated(_ layout: ContainerViewLayout, transition: ContainedViewLayoutTransition) {
        ...
        (self.displayNode as! WalletApplicationSplashScreenNode).containerLayoutUpdated(layout: layout, navigationHeight: self.cleanNavigationHeight, transition: transition)
    }
