...
public final class WalletSplashScreen: ViewController {
    ...
    public init(context: WalletContext, blockchainNetwork: LocalWalletConfiguration.ActiveNetwork, mode: WalletSplashMode, walletCreatedPreloadState: Promise<WalletCreatedPreloadState?>?) {
        ...
        super.init(navigationBarPresentationData: NavigationBarPresentationData(theme: navigationBarTheme, strings: NavigationBarStrings(back: self.presentationData.walletStrings.Wallet_Intro_NotNow, close: self.presentationData.walletStrings.Wallet_Navigation_Close)))
        ...
        switch self.mode {
        case .intro:
            self.navigationItem.setRightBarButton(UIBarButtonItem(title: self.presentationData.walletStrings.Wallet_Intro_ImportExisting, style: .plain, target: self, action: #selector(self.importPressed)), animated: false)
        case let .sending(sending):
            ...
            |> deliverOnMainQueue).start(next: { [weak self] decryptedSecret in
                ...
                if case .cancelled = error {
                    ...
                    let text = strongSelf.presentationData.walletStrings.Wallet_Send_ErrorDecryptionFailed
                    let controller = standardTextAlertController(theme: .init(presentationData: strongSelf.context.presentationData), title: nil, text: text, actions: [TextAlertAction(type: .defaultAction, title: strongSelf.presentationData.walletStrings.Wallet_Alert_OK, action: {
                        self?.dismiss()
                    })])
    ...
    private func sendGrams(walletInfo: WalletInfo, decryptedSecret: Data, address: String, amount: Int64, comment: Data, encryptComment: Bool, forceIfDestinationNotInitialized: Bool, randomId: Int64, serverSalt: Data) {
        ...
        |> deliverOnMainQueue).start(next: { [weak self] sentTransaction in
            ...
            strongSelf.navigationItem.setRightBarButton(UIBarButtonItem(title: strongSelf.presentationData.walletStrings.Wallet_WordImport_Continue, style: .plain, target: strongSelf, action: #selector(strongSelf.sendGramsContinuePressed)), animated: false)
            ...
            switch error {
            case .generic:
                text = strongSelf.presentationData.walletStrings.Wallet_UnknownError
            case .network:
                title = strongSelf.presentationData.walletStrings.Wallet_Send_NetworkErrorTitle
                text = strongSelf.presentationData.walletStrings.Wallet_Send_NetworkErrorText
            case .notEnoughFunds:
                title = strongSelf.presentationData.walletStrings.Wallet_Send_ErrorNotEnoughFundsTitle
                text = strongSelf.presentationData.walletStrings.Wallet_Send_ErrorNotEnoughFundsText
            case .messageTooLong:
                text = strongSelf.presentationData.walletStrings.Wallet_UnknownError
            case .invalidAddress:
                text = strongSelf.presentationData.walletStrings.Wallet_Send_ErrorInvalidAddress
            case .secretDecryptionFailed:
                text = strongSelf.presentationData.walletStrings.Wallet_Send_ErrorDecryptionFailed
            case .destinationIsNotInitialized:
                if !forceIfDestinationNotInitialized {
                    text = strongSelf.presentationData.walletStrings.Wallet_Send_UninitializedText
                    let controller = standardTextAlertController(theme: .init(presentationData: strongSelf.context.presentationData), title: strongSelf.presentationData.walletStrings.Wallet_Send_UninitializedTitle, text: text, actions: [
                        TextAlertAction(type: .genericAction, title: strongSelf.presentationData.walletStrings.Wallet_Navigation_Cancel, action: {
                            if let navigationController = strongSelf.navigationController as? NavigationController {
            ...
            let controller = standardTextAlertController(theme: .init(presentationData: strongSelf.presentationData), title: title, text: text, actions: [TextAlertAction(type: .defaultAction, title: strongSelf.presentationData.walletStrings.Wallet_Alert_OK, action: {
                if let navigationController = strongSelf.navigationController as? NavigationController {
                    let _ = navigationController.popViewController(animated: true)
                }
            })])
        ...
        self.displayNode = WalletSplashScreenNode(context: self.context, walletCreatedPreloadState: self.walletCreatedPreloadState, presentationData: self.presentationData, mode: self.mode, action: { [weak self] in
            ...
            case .intro:
                ...
                let displayError: () -> Void = {
                    ...
                    controller.dismiss()
                    strongSelf.present(standardTextAlertController(theme: .init(presentationData: strongSelf.presentationData), title: strongSelf.presentationData.walletStrings.Wallet_Intro_CreateErrorTitle, text: strongSelf.presentationData.walletStrings.Wallet_Intro_CreateErrorText, actions: [
                    ...
                    |> deliverOnMainQueue).start(next: { [weak controller] decryptedSecret in
                        ...
                        |> deliverOnMainQueue).start(next: { [weak controller] serverSalt in
                            ...
                            |> deliverOnMainQueue).start(next: { wordList in
                                ...
                                strongSelf.present(standardTextAlertController(theme: .init(presentationData: strongSelf.presentationData), title: strongSelf.presentationData.walletStrings.Wallet_Created_ExportErrorTitle, text: strongSelf.presentationData.walletStrings.Wallet_Created_ExportErrorText, actions: [
                                    TextAlertAction(type: .defaultAction, title: strongSelf.presentationData.walletStrings.Wallet_Alert_OK, action: {
                                    })
                                ], actionLayout: .vertical), in: .window(.root))
                            })
    ...
    override public func containerLayoutUpdated(_ layout: ContainerViewLayout, transition: ContainedViewLayoutTransition) {
        ...
        (self.displayNode as! WalletSplashScreenNode).containerLayoutUpdated(layout: layout, navigationHeight: self.cleanNavigationHeight, transition: transition)
    }
}
...
private final class WalletSplashScreenNode: ViewControllerTracingNode {
    ...
    init(context: WalletContext, walletCreatedPreloadState: Promise<WalletCreatedPreloadState?>?, presentationData: PresentationData, mode: WalletSplashMode, action: @escaping () -> Void, secondaryAction: @escaping () -> Void, openTerms: @escaping () -> Void) {
        ...
        switch mode {
        case .intro:
            title = self.presentationData.walletStrings.Wallet_Intro_Title
            text = NSAttributedString(string: self.presentationData.walletStrings.Wallet_Intro_Text, font: textFont, textColor: textColor)
            buttonText = self.presentationData.walletStrings.Wallet_Intro_CreateWallet
            ...
            if let _ = context.termsUrl {
                termsText = parseMarkdownIntoAttributedString(self.presentationData.walletStrings.Wallet_Intro_Terms, attributes: MarkdownAttributes(body: body, bold: body, link: link, linkAttribute: { _ in nil }), textAlignment: .center)
            } else {
                termsText = NSAttributedString(string: "")
            }
            self.iconNode.image = nil
            self.animationNode.setup(source: AnimatedStickerNodeLocalFileSource(name: "WalletIntroLoading"), width: 248, height: 248, mode: .direct(cachePathPrefix: nil))
            self.animationSize = CGSize(width: 124.0, height: 124.0)
            self.animationNode.visibility = true
            secondaryActionText = ""
        case .created:
            title = self.presentationData.walletStrings.Wallet_Created_Title
            text = NSAttributedString(string: self.presentationData.walletStrings.Wallet_Created_Text, font: textFont, textColor: textColor)
            buttonText = self.presentationData.walletStrings.Wallet_Created_Proceed
            termsText = NSAttributedString(string: "")
            self.iconNode.image = nil
            self.animationNode.setup(source: AnimatedStickerNodeLocalFileSource(name: "WalletCreated"), width: 250, height: 250, playbackMode: .once, mode: .direct(cachePathPrefix: nil))
            self.animationSize = CGSize(width: 125.0, height: 125.0)
            self.animationNode.visibility = true
            secondaryActionText = ""
        case .successfullyCreated, .successfullyImported:
            title = self.presentationData.walletStrings.Wallet_Completed_Title
            text = NSAttributedString(string: self.presentationData.walletStrings.Wallet_Completed_Text, font: textFont, textColor: textColor)
            buttonText = self.presentationData.walletStrings.Wallet_Completed_ViewWallet
            termsText = NSAttributedString(string: "")
            self.iconNode.image = nil
            self.animationNode.setup(source: AnimatedStickerNodeLocalFileSource(name: "WalletDone"), width: 260, height: 260, playbackMode: .loop, mode: .direct(cachePathPrefix: nil))
            self.animationSize = CGSize(width: 130.0, height: 130.0)
            self.animationOffset = CGPoint(x: 0.0, y: 0.0)
            self.animationNode.visibility = true
            secondaryActionText = ""
        case .restoreFailed:
            title = self.presentationData.walletStrings.Wallet_RestoreFailed_Title
            text = NSAttributedString(string: self.presentationData.walletStrings.Wallet_RestoreFailed_Text, font: textFont, textColor: textColor)
            buttonText = self.presentationData.walletStrings.Wallet_RestoreFailed_CreateWallet
            termsText = NSAttributedString(string: "")
            self.iconNode.image = nil
            self.animationNode.setup(source: AnimatedStickerNodeLocalFileSource(name: "WalletNotAvailable"), width: 260, height: 260, playbackMode: .once, mode: .direct(cachePathPrefix: nil))
            self.animationSize = CGSize(width: 130.0, height: 130.0)
            self.animationNode.visibility = true
            secondaryActionText = self.presentationData.walletStrings.Wallet_RestoreFailed_EnterWords
        case .sending:
            title = self.presentationData.walletStrings.Wallet_Sending_Title
            text = NSAttributedString(string: self.presentationData.walletStrings.Wallet_Sending_Text, font: textFont, textColor: textColor)
            buttonText = self.presentationData.walletStrings.Wallet_Sent_Title
            buttonHidden = true
            termsText = NSAttributedString(string: "")
            self.iconNode.image = nil
            self.animationNode.setup(source: AnimatedStickerNodeLocalFileSource(name: "SendingGrams"), width: 260, height: 260, mode: .direct(cachePathPrefix: nil))
            self.animationSize = CGSize(width: 130.0, height: 130.0)
            self.animationNode.visibility = true
            secondaryActionText = ""
        case let .sent(_, amount):
            title = self.presentationData.walletStrings.Wallet_Sent_Title
            ...
            text = parseMarkdownIntoAttributedString(self.presentationData.walletStrings.Wallet_Sent_Text(formatBalanceText(amount, decimalSeparator: self.presentationData.dateTimeFormat.decimalSeparator)).0, attributes: MarkdownAttributes(body: bodyAttributes, bold: boldAttributes, link: bodyAttributes, linkAttribute: { _ in return nil }), textAlignment: .center)
            buttonText = self.presentationData.walletStrings.Wallet_Sent_ViewWallet
            termsText = NSAttributedString(string: "")
            self.iconNode.image = nil
            self.animationNode.setup(source: AnimatedStickerNodeLocalFileSource(name: "WalletDone"), width: 260, height: 260, playbackMode: .once, mode: .direct(cachePathPrefix: nil))
            self.animationSize = CGSize(width: 130.0, height: 130.0)
            self.animationOffset = CGPoint(x: 14.0, y: 0.0)
            self.animationNode.visibility = true
            secondaryActionText = ""
        case .secureStorageNotAvailable:
            title = self.presentationData.walletStrings.Wallet_SecureStorageNotAvailable_Title
            text = NSAttributedString(string: self.presentationData.walletStrings.Wallet_SecureStorageNotAvailable_Text, font: textFont, textColor: textColor)
            buttonText = self.presentationData.walletStrings.Wallet_Alert_OK
            termsText = NSAttributedString(string: "")
            self.iconNode.image = nil
            self.animationNode.setup(source: AnimatedStickerNodeLocalFileSource(name: "WalletKeyLock"), width: 280, height: 280, playbackMode: .once, mode: .direct(cachePathPrefix: nil))
            self.animationSize = CGSize(width: 140.0, height: 140.0)
            self.animationNode.visibility = true
            secondaryActionText = ""
        case let .secureStorageReset(reason):
            title = self.presentationData.walletStrings.Wallet_SecureStorageReset_Title
            
            let biometricTypeString: String?
            if let type = LocalAuth.biometricAuthentication {
                switch type {
                case .faceId:
                    biometricTypeString = self.presentationData.walletStrings.Wallet_SecureStorageReset_BiometryFaceId
                case .touchId:
                    biometricTypeString = self.presentationData.walletStrings.Wallet_SecureStorageReset_BiometryTouchId
                }
            } else {
            ...
            case .notAvailable:
                ...
                if let biometricTypeString = biometricTypeString {
                    string = self.presentationData.walletStrings.Wallet_SecureStorageReset_BiometryText(biometricTypeString).0
                } else {
                    string = self.presentationData.walletStrings.Wallet_SecureStorageReset_PasscodeText
                }
            ...
            case .changed:
                let string: String
                if let biometricTypeString = biometricTypeString {
                    string = self.presentationData.walletStrings.Wallet_SecureStorageChanged_BiometryText(biometricTypeString).0
                } else {
                    string = self.presentationData.walletStrings.Wallet_SecureStorageChanged_PasscodeText
                }
                text = NSAttributedString(string: string, font: textFont, textColor: textColor)
                buttonText = self.presentationData.walletStrings.Wallet_SecureStorageChanged_ImportWallet
                secondaryActionText = self.presentationData.walletStrings.Wallet_SecureStorageChanged_CreateWallet
            }
            termsText = NSAttributedString(string: "")
            self.iconNode.image = nil
            self.animationNode.setup(source: AnimatedStickerNodeLocalFileSource(name: "WalletNotAvailable"), width: 260, height: 260, playbackMode: .once, mode: .direct(cachePathPrefix: nil))
